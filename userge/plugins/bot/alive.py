...
if userge.has_bot:
    ...
    async def alive_cb(_, c_q: CallbackQuery):
        ...
        alive_s += f"🔌 𝘿𝙧𝙖𝙜𝙤𝙣 : {_parse_arg(Config.SUDO_ENABLED)}\n"
        alive_s += f"🎭 𝐀𝐧𝐭𝐢𝐯𝐢𝐫𝐮𝐬 : {_parse_arg(Config.ANTISPAM_SENTRY)}\n"
        if Config.HEROKU_APP and Config.RUN_DYNO_SAVER:
            alive_s += "🔋 𝐂𝐫𝐞𝐝𝐢𝐭 𝐒𝐚𝐯𝐞𝐫 :  ✅ 𝙴𝚗𝚊𝚋𝚕𝚎𝚍\n"
        alive_s += f"💡 𝐆𝐨𝐝𝐳𝐢𝐥𝐥𝐚 𝐅𝐨𝐫𝐰𝐚𝐫𝐝𝐬 : {_parse_arg(Config.BOT_FORWARDS)}\n"
        alive_s += f"🛡 𝐏𝐫𝐨𝐭𝐞𝐜𝐭 𝐈𝐧𝐛𝐨𝐱 : {_parse_arg(not Config.ALLOW_ALL_PMS)}\n"
        alive_s += f"📝 𝐈𝐧𝐛𝐨𝐱 𝐋𝐨𝐠𝐠𝐞𝐫 : {_parse_arg(Config.PM_LOGGING)}"
        if allow:
            end = datetime.now()
